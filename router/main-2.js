...
// відображаємо сторінку шаблонізатора index 
router.get('/', (req, res) => {
res.render('index');
});
// наші дані
...
// створюємо масив з яким проводити маніпулювання
const myArry = [ ...array];

// відправляємо дані сервера з констатнти myArry по запросу на роут 
router.get('/name', (req, res) => {
    res.json(myArry);
    });

// відправляємо дані з сервера та ті що ввів користувач з констатнти myArry по запросу на роут
...
//експортуємо наш роут до файла app.js та підключаєго його там
